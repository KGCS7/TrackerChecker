pub mod p_hashing {
    ...
    pub fn add_p_hash_for_media(p_hash_this: &std::path::Path) {
        ...
        match p_hash_this.extension()
            .expect("Error extracting the file extension")
            .to_str()
            .expect("Error converting the file extension into a string") {
            "png"|"jpg"|"jpeg"=>{
                match std::fs::copy(p_hash_this, temp_dir.join(p_hash_this.file_name().expect("msg"))) {
                    Ok(transfered_file)=>{
                        if !transfered_file.ge(&0) {std::println!("No bytes transferred");}
                    }
                    Err(e)=>{std::println!("Error in transferring the file {:#?}: {e}", &p_hash_this.file_name());}
                };
                    ...
                    match decode_context.receive_frame() {
                        Ok(frame) => {
                            ...
                            match image::save_buffer_with_format(
                                &out_path, &dst_buf, width as u32, height as u32,
                                image::ColorType::Rgb8,image::ImageFormat::Png,
                            ) {
                                Ok(saved_image) => {saved_image}
                                Err(e) => {std::println!("Error saving image: {e}"); continue;}
                            }
                        ...
                        Err(rsmpeg::error::RsmpegError::DecoderFlushedError) | Err(_) => {continue;} // Load Bearing
                        Err(e) => {std::println!("Error receiving frame from decoder: {e}"); break;}
                    }
                }
    ...
    compare_hashes_parallel(&processed_images[..]);
}

    fn process_entries_parallel(entries: &[std::fs::DirEntry]) -> Vec<p_hash_data> {
        if entries.is_empty() {
            return Vec::new();
        }
        if entries.len() == 1 {
            ...
            return vec![p_hash_data{image_path:path,image_d_hash:img_hash}];
        }
    ...
    fn compare_hashes_parallel(images: &[p_hash_data]) {
        compare_recursive(images, images);
    }

    fn compare_recursive(full_list: &[p_hash_data], targets: &[p_hash_data]) {
        if targets.is_empty() {
            return;
        }
        ...
        if targets.len() == 1 {
            ...
            for comp in full_list {
                if std::ptr::eq(target, comp) {continue; }

                let h_d = target.image_d_hash.hamming_distance(&comp.image_d_hash);
                let float: f64 = 1.0 - (h_d as f64 / 64.0);

                if float >= 0.90 {
                    println!(
                        "\tPossible Duplicate found for file {:?} (matches {:?})",
                        target.image_path, comp.image_path
                    );
                }
            }
            return;
        ...
        rayon::join(
            || compare_recursive(full_list, left),
            || compare_recursive(full_list, right),
        );
    }
...
pub mod misc {
    ...
    pub fn flush_the_cache(mode:bool){
        if mode.eq(&true) {
            match  std::fs::remove_dir_all(std::path::Path::new("./tempDir/")){
                ...
                Err(e)=>{std::println!("Error deleting the cached files {e}.");}
            }
            match std::fs::DirBuilder::new().recursive(true).create(std::path::Path::new("./tempDir/")){
                Ok(d)=>{d}
                Err(e)=>{std::println!("Error in creating the tempDir: {e}"); return;}
            };
        }else {
            match  std::fs::remove_dir_all(std::path::Path::new("./tempDir/")){
                Ok(_)=>{std::println!("Removed the cached.");}
                ...
                if file_descriptor_info.is_dir() {
                    match std::fs::read_dir(walk_thru) {
                        Ok(go_thru) => {
                            for file in go_thru {
                                match file {
                                    Ok(try_this) => {go_thru_dir(&mut try_this.path(), add_here, method);}
                                    Err(e) => {std::println!("Error {}", e);}
                                }
                            }
                        }
                    ...
                    match std::fs::File::open(&walk_thru) {
                        Ok(_) => {
                            if method.eq("p") {utils::p_hashing::add_p_hash_for_media(&walk_thru);} 
                            else if method.eq("b") {utils::s_hashing::add_hash_from_path(&walk_thru, add_here);} 
                            else {std::println!("Unable to match the method {:#?} with one of the options",method);}
                        }
    ...
    pub fn _find_and_set_ram_limit() {
        #[cfg(any(target_os = "macos", target_os = "linux"))]
        {
            ...
            let set_the_limit = unsafe { libc::setrlimit(libc::RLIMIT_DATA, &mut sys_mut_lim_struct) };
            match set_the_limit {
                0 => {std::println!("Limit has been properly set {:#?}", sys_mut_lim_struct)}
                -1 => {std::println!( "An Error has occurred. Try Debugging {:#?}", std::io::Error::last_os_error().raw_os_error())}
                _ => {std::println!("Something went horribly wrong!");}
            }
